        ...
        <Container>
          <div className="flex flex-col gap-5 border-y border-gold/25 py-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex gap-2 overflow-x-auto pb-2 lg:pb-0">
              {categories.map((item) => <button className={`focus-ring shrink-0 rounded-md px-4 py-2.5 text-xs font-bold ${category === item.value ? 'bg-gold text-white' : 'border border-gold/30 bg-parchment/70 text-episcopal hover:bg-white/75'}`} key={item.label} onClick={() => setCategory(item.value)} type="button">{localize(item.label)}</button>)}
            </div>
