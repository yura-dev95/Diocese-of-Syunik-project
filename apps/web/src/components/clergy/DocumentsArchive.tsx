...
export function DocumentsArchive() {
  ...
  return <div className="grid gap-4">{data.map((document) => <article className="flex flex-col gap-5 border border-gold/25 bg-white/45 p-6 sm:flex-row sm:items-center sm:justify-between" key={document.id}><div className="flex gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-full bg-episcopal text-gold"><FileText className="size-5" /></span><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-royal">{document.documentType} · {new Intl.DateTimeFormat('hy-AM').format(new Date(document.documentDate))}</p><h3 className="mt-2 font-display text-xl font-bold text-episcopal">{document.title}</h3><p className="mt-2 text-sm leading-6 text-ink/55">{document.description}</p></div></div><button className="focus-ring inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-full border border-gold/40 px-4 text-xs font-bold text-episcopal" type="button"><Download className="size-4" /> Ներբեռնել</button></article>)}</div>;
}
