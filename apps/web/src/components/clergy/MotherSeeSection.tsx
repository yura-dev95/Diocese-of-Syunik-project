...
export function MotherSeeSection() {
  return (
    <section className="py-24 sm:py-32"><Container className="grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-start">
      ...
      <div className="grid gap-3">{links.map(([label, href]) => <AnimatedCard className="group border border-gold/25 bg-parchment/80" key={label}><a className="focus-ring flex items-center justify-between gap-5 p-5 text-sm font-bold text-episcopal transition hover:bg-white/75" href={href} rel="noreferrer" target="_blank"><span>{label}</span><ArrowUpRight className="size-4 shrink-0 text-royal transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></a></AnimatedCard>)}</div>
    </Container></section>
  );
}
