  ...
  return (
    <div className="flex flex-col gap-5 border-y border-gold/25 py-6 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex gap-2 overflow-x-auto pb-2 lg:pb-0">
        <button
          className={`focus-ring shrink-0 rounded-full px-4 py-2.5 text-xs font-bold transition ${!category ? 'bg-episcopal text-parchment' : 'border border-gold/30 text-episcopal hover:bg-gold/10'}`}
          type="button"
        ...
        {(Object.entries(churchCategoryLabels) as [ChurchCategory, string][]).map(([value, label]) => (
          <button
            className={`focus-ring shrink-0 rounded-full px-4 py-2.5 text-xs font-bold transition ${category === value ? 'bg-episcopal text-parchment' : 'border border-gold/30 text-episcopal hover:bg-gold/10'}`}
            key={value}
            type="button"
      ...
      <label className="relative block min-w-64">
        ...
        <input
          className="focus-ring h-12 w-full rounded-full border border-gold/35 bg-white/45 pl-11 pr-4 text-sm text-episcopal placeholder:text-ink/40"
