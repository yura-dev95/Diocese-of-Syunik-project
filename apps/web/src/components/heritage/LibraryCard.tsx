  ...
  return (
    <AnimatedCard className="group overflow-hidden border border-gold/25 bg-parchment/80 transition hover:-translate-y-1 hover:shadow-sacred">
      <div className="relative aspect-[4/3] overflow-hidden bg-episcopal">
        ...
        <div className="absolute left-4 top-4"><Badge>{localize(labels[item.category])}</Badge></div>
        <Icon className="absolute bottom-5 left-5 size-8 text-gold" strokeWidth={1.3} />
      </div>
      <div className="p-6">
        <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-royal/65">{localize(item.author ?? 'Սյունյաց թեմի արխիվ')}</p>
        <h3 className="mt-3 font-display text-2xl font-bold text-episcopal">{localize(item.title)}</h3>
        <p className="mt-3 line-clamp-2 text-sm leading-7 text-ink/60">{localize(item.description)}</p>
        <div className="mt-6 flex items-center justify-between border-t border-gold/20 pt-4 text-xs text-ink/50">
          <span>{item.publicationYear} · {item.pageCount} {localize('էջ')}</span>
