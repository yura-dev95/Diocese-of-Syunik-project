...
export function LoadingState({ count = 3, variant = 'cards' }: LoadingStateProps) {
  ...
  if (variant === 'rows') {
    return (
      <div className="grid gap-3" aria-label={t('common.loading')} aria-live="polite">
        {items.map((item) => (
          <div className="animate-pulse rounded-2xl border border-gold/15 bg-white/45 p-5" key={item}>
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3" aria-label={t('common.loading')} aria-live="polite">
      {items.map((item) => (
        <div className="animate-pulse overflow-hidden rounded-t-[3rem] border border-gold/20 bg-white/40 shadow-sm" key={item}>
